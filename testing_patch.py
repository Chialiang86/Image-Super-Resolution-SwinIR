        ...
        for hi in range(hh_num):
            for wi in range(hw_num):
                ...
                # case 2 
                    ...
                    hr_crop = hr_img[-args.size:, (wi * args.size): (wi * args.size + args.size)]
                # case 3 
                elif wi == hw_num - 1:
                    hr_crop = hr_img[(hi * args.size): (hi * args.size + args.size), -args.size:]
                # case 4 
                else :
                    hr_crop = hr_img[(hi * args.size): (hi * args.size + args.size), (wi * args.size): (wi * args.size + args.size)]
                cv2.imwrite(f'{dst_path}/{img_file[:-4]}_{size[0]}_{size[1]}_{lid}.png', hr_crop)
                lid += 1
