        ...
        if self.opt['phase'] == 'train':
            ...
            if r_value > 3:
                ...
                k = utils_sisr.gen_kernel(
                    scale_factor=np.array([sf_k, sf_k]))  # Gaussian blur
            ...
            img_L = ndimage.filters.convolve(
                img_H, np.expand_dims(
                    k, axis=2), mode='wrap')  # blur
            img_L = img_L[0::self.sf_validation,
                          0::self.sf_validation, ...]  # downsampling
            img_L = util.uint2single(
                img_L) + np.random.normal(0, noise_level, img_L.shape)
